 ...
 */
module scene {
    export class SendCardAniProxy implements IInit, IRelease {
        ...
        public StartAni(player: data.Player, overfun: Function, thisObj: any) {
        ...
        private sendCardAni(): void {
            ...
            var i: number = 0;
            for (i = 0; i < 3; i++) {
                var card: Card = MandPool.getInsByParm(Card, 0);
                this._sendLayer.addChild(card);
                card.x = 220 + i * 10;
                card.y = -300;
            }
            this._mCard1 = MandPool.getInsByParm(Card, 0);
            this._cardLayer.addChild(this._mCard1);
            this._mCard1.x = 250;
            this._mCard1.y = -300;
            this._mCard2 = MandPool.getInsByParm(Card, 0);
            this._cardLayer.addChild(this._mCard2);
            this._mCard2.x = 250;
            this._mCard2.y = -300;
            this._mCard3 = MandPool.getInsByParm(Card, 0);
        ...
        private onTimer(e: egret.TimerEvent): void {
            ...
            if (ccount == this._timer1.repeatCount) {
                ...
                var arr: Array<number> = this._player.CardArr.slice(0, ccount);
                this.setCard(arr);
            }
        }
